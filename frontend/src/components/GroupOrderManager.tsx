...
export const GroupOrderManager: React.FC<GroupOrderManagerProps> = ({
  ...
  if (!currentSession) {
    return (
      <div className="bg-stone-900 border border-stone-850 p-6 rounded-[32px] shadow-xl w-full scroll-reveal">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center w-full gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-stone-100 uppercase tracking-wider">Group Ordering & Bill Split</h3>
              <p className="text-[10px] text-stone-500 font-semibold mt-0.5">Order together with friends and split bills instantly</p>
            </div>
          </div>
          <button 
            onClick={() => setIsCollapsed(!isCollapsed)}
            className="text-[10px] font-black bg-stone-950 hover:bg-stone-900 border border-stone-800 hover:border-stone-700 text-amber-500 px-4 py-2.5 rounded-xl cursor-pointer transition-all flex items-center gap-1.5 shrink-0 self-stretch sm:self-auto justify-center"
          >
            {isCollapsed ? 'Open Setup' : 'Close Setup'}
          </button>
        </div>

        {!isCollapsed && (
          <div className="mt-5 pt-5 border-t border-stone-850 animate-[fadeIn_0.3s_ease-out] flex flex-col gap-4">
            <p className="text-[11px] text-stone-400 leading-relaxed">
              Sync baskets in real-time across screens. Add custom names, invite participants via sharing links, and see real-time division options before ordering.
            </p>
              ...
              <form onSubmit={handleJoin} className="flex flex-col gap-3 border-t md:border-t-0 md:border-l border-stone-850 pt-4 md:pt-0 md:pl-6">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="w-20 px-3 py-2.5 rounded-xl bg-stone-950 border border-stone-850 text-xs font-semibold text-stone-300 focus:outline-none focus:border-amber-500 transition-colors"
                  />
                  <input
                    type="text"
